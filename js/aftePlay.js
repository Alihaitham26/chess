...
function afterPlay() {
    for (let key in specialMoves) {
        if (specialMoves[key].condition(selectedPiece.position)) {
            specialMoves[key].action(selectedPiece.position)
        }
    }
    specialMoves = {}
    ...
    */
    ...
    for (let team of teams) {
        for (let key in team) {
            ...
            if (piece.type === "pawn" && piece.position[1] === (piece.isWhite ? "8" : "1")) {
                upgradePawn(piece)
            }
